...
Since `+UseContainerSupport` is enabled by default, everything should work out if the box. You may want to fine tune  `-XX:MaxRAMPercentage` to be both _efficient and safe_, depending on your environment. But the new parameters default to a good compromise and are easier to understand. That's a huge step in the right direction.
...
[^maxram]: Please note that the MaxRAM value seems broken for my machine as it shows 128GB MaxRAM, which is wrong. The resulting heap, however, is correct (32*1/4 == 8).
